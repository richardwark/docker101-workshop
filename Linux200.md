...
# Start up and login into your Liunx environment
...
**NOTE: You only have to set up ONE database (Oracle or MYSQL) yo use with the AlphaOffice application but can go through the setup of both if you'd like.**
...
This docker command will create a container base on the database image file located in the wvbirder/database-enterprise repository having the tag: 12.2.0.1-slim. 
...
Let's take a look at what the docker **run** command options do:
  ...
  "-v" Thhis maps the directory where you downloaded the AlphaOfficeSetup GIT
    repo to the /dbfiles directory within the container 

- Type OR cut and paste the following: **docker run -d -it --name orcl -h='oracledb-ao' -p=1521:1521 -p=5600:5600 -v /YOUR-HOME/AlphaOfficeSetup:/dbfiles wvbirder/database-enterprise:12.2.0.1-slim**
...
In this step we will connect into the database container and run script to create the alpha schema user and populate the Product Catalog tables.
...
If there are no permissions erros the "xxx" file should be present in the directory
...
**NOTE:** If you want to login to Enterprise Manager Express the browser need the Shockwave add-on. Install this into your browser environment by going to the adobe webite and downloading the player from: https://get.adobe.com/shockwave/

**If you are using the VirtualBox VM for this worshop Shockwave has already been installed.**
...
This docker command will create a container base on the latest MYSQL database image file located in Docker Hub
...
- Type OR cut and paste the following: **docker run -d -it --name mysql -h='mysqldb-ao' -p=3306:3306 -v /u01/AlphaOfficeSetup:/dbfiles --env="MYSQL_ROOT_PASSWORD=Alpha2017_" mysql**
...
In this step we will connect into the database container and run script to create the AlphaOfficeDB and then create the database user and populate the Product Catalog tables.
...
If there are no permissions erros the "yyy" file should be present in the directory
...
- TwitterFeed: This java application makes static Twitter post (via a JSON file) available via REST calls. The AlphaOffice UI mmakes calls to it and associates the twitter posts to Products displayed in the UI app.
...
- AlphaOfficeUI: Node.js application displays the data from a combination of the TwitterFeed and ClientREST application.  

### **STEP 1**: Run and test the TwitterFeed

- Type OR cut and paste: **docker run -d --name=twitterfeed -p=9080:9080 wvbirder/twitterfeed**

The images will download the first time, extract and run the container.
...
**NOTE:** The firefox browser included in the Linux VirtualBox VM has a built-in JSON formatter. If you don't you'll see a stream of text representing the tweets.
...
Let's take a look at what the docker **run** command options do:
  ...
  "--link" A intra-container link to the "mysql" database is created using the "mysqldb" hostname. This hostname is added to the container's /etc/host file. Hostname is used because it is more flexible than using the private IP address of the container as that can change upon subsquent invocations. The hostname is used for making a connection to the database 

  "-e" Enviornment variables used by the application. "MYSQL_HOST" and "DS" settings designate the MYSQL datasource. The Oracle datsource uses the ORACLE_CONNECT variable (an example of that is shown below)

**NOTE:** This example assumes we are using the MYSQL database. 
...
A list od ALL products are shown:

![](images/200Linux/Picture200-24.png)

You can also query and individual product. In the browser, open up a new tab and enter: **http://localhost:8002/product/1025**
...
**OPTONAL:** If you configured both ORACLE and MYSQL databases then you can stop the "restclient" container after testing with one datasource by typing: **docker stop (name-of-container)**. Then, start the "restclient" container stipulating the new datasource using the appropriate commands already shown above.
...
In this section you make a couple of changes to the AlphaOfficeUI application. One will correct a typo and another will change the background image.

- Copy the new background image into the container
- Connect into the AlphaOfficeUI container
- install the vim text editor
- Fix a typo
- Save (commit) a copy of the changes to a NEW docker image
...
Copy a background image file into the running AlphaOfficeUI container. This file is in your $HOME/AlphaOfficeSetup directory that you GIT cloned at the beginning of the lab

- Type **docker cp $HOME/AlphaOfficeSetup/dark_blue.jpg alphaofficeui:/pipeline/source/public/Images**

### **STEP 2**: Install the VIM editor in the container

Even though the orginal AlphaOfficeUI image could have been set up ahead of time with anu needed client tools we're adding the the environment on-the-fly to give you some idea that it can be done
...
- Edit the "alpha.html" file to fix a typo: **vim /pipeline/source/public/alpha.html**
  ...
  If you want you can also change the body title as shown below:
...
In this step you will save a copy of your modifed docker container and give it a new name

- Back out in the HOST type **docker commit alphaofficeui (your-dockerhub-account)/(image-name)**
...
Since there is already a running alphaofficeui container we'll name the new container alphaofficeui2 and use port 8086 on the HOST since 8085 is already being used

- Type **docker run -d --name=alphaofficeui2 -p=8086:8085 (your-dockerhub-account)/alphaoffice-new**

**NOTE:** You could have stopped the original container by typing "docker stop alphaofficeui" and you wouldn't have had to use a different network port
...
Now others will be able to take advantage of the changes you made to the application. In this example "wvbirder" is the Docker Hub but you will be using your account
